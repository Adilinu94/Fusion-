...
Grundlage sind die Architekturregeln in [`docs/ARCHITEKTURREGELN.md`](docs/ARCHITEKTURREGELN.md)
(Rekonstruktion des verbindlichen technischen Bauplans, Stand 27.07.2026 —
das Originaldokument existiert nicht im Repo, siehe Verbesserungsplan
B-DOC-1). Abweichungen von den Regeln sind nur ueber ADRs in
[`docs/adr/`](docs/adr/) erlaubt.
...
| 5 (Schritt 19) | Pro-Ausgang-Profile, Bit-Perfect (USB, Android 14+), BT-Anzeige | Teilweise. Abgeschlossen: Profile je Geraet (`DeviceProfileStore` + `OutputProfileController`, automatischer Wechsel + Save-Through), Vertrag `activeOutputProfileKey`/`bitPerfectSupport`; UI in `:feature:audio` (Bit-Perfect-Panel mit DAC-Faehigkeiten, aktives Ausgabeprofil, BT-Hinweis + Link zu den System-Toneinstellungen), erreichbar ueber Einstellungen -> Audio & DSP. **Offen: der Bit-Perfect-Modus laesst sich nicht einschalten.** `BitPerfectGateway` liest die Faehigkeiten (`getSupportedMixerAttributes`, API 34+), ruft aber nie `setPreferredMixerAttributes` — der Ausgang wird also nie umgestellt; `floatOutput` steht in `PlaybackService` hart auf `false`. Was funktioniert, ist die Erkennung plus der DSP-Bypass, nicht der bitgenaue Pfad. **B-AUD-4 (Weg b, Nutzerentscheidung 2026-09-11):** der UI-Text im Bit-Perfect-Panel nennt diese Grenze explizit ("umgeht derzeit nur die DSP-Kette; bitgenaue Mixer-Ausgabe ist noch nicht umgesetzt") statt eine Wirkung zu versprechen, die es nicht gibt |
...
| 7 (Schritt 21) | Feinschliff, Barrierefreiheit, Performance, Geraetetests | Codeseitig abgeschlossen: Barrierefreiheit (Slider mit `stateDescription`, 48-dp-Schaltflaechen, Schaltererklaerungen), DE/EN-Strings, Akku-Hinweis bei Hi-Res-Resampling, DSP-Durchsatz-/Stabilitaetswaechter (`DspPerformanceTest`: 32 Baender/48 kHz > 2x Echtzeit, keine NaN/Inf). **Baseline Profiles sind vollstaendig verdrahtet (B-UI-5, 2026-09-03):** `BaselineProfileGenerator` in `:benchmarks` erzeugt das Profil, `baselineProfile(project(":benchmarks"))` in `:app` verbindet Producer und Consumer, `StartupBenchmark` misst gegen `CompilationMode.Partial(BaselineProfileMode.Require)`. Beim Umsetzen kam heraus, dass `:benchmarks` seit `cb7efda` nicht kompilierte (fehlende `junit4`/`androidx.test.ext.junit`, entfernte `CompilationMode.BaselineProfile()`-API, ueberfluessiges `kotlin.compose`) - die CI baut `com.android.test`-Module nicht und hat es deshalb nie gemeldet. Der Generatorlauf braucht ein Geraet (Root oder API 33+, Emulator mit `aosp`-Abbild). Reale CPU-Messung Mittelklasse, USB-DAC-Bit-Perfect, BT-Codec-Verhalten, MusicFX mit/ohne Systemequalizer bleiben Geraeteabnahme |
...
| Phase | Inhalt | Status |
| ----- | ------ | ------ |
| 1 | BPM-/Tonart-Analyse additiv im bestehenden `TrackAnalyzer`-Durchgang (`track_analysis.bpm`/`camelot_key`), Migration v4->v5 | Abgeschlossen (nicht "Entwurf", wie bis 04.09.2026 hier stand): Spalten `bpm`/`camelot_key` liegen seit Migration v6->v7 in `track_analysis`, `TempoAccumulator`/`ChromaAccumulator` laufen additiv in `TrackAnalyzerImpl`, das Konfidenz-Gate steht in ADR-0019. Seit ADR-0015 laeuft die Stufe als eigenes Profil `MIX_METADATA` mit getrennter Cache-Version |
| 2 | `MixPreset`-Enum (Fade/Rise/Blend/Wave/Melt/Slam) als Volume-Kurven mit Equal-Power-Invariante (`sqrt(1 - fadeIn^2)`, Basis `CrossfadeCurves` in `:domain:audio`), SLAM-Klickschutz per Mikro-Rampe, `DspConfig.mixPreset` (DataStore + Profil-Codec) | Kurven und Persistenz abgeschlossen, **Anwendung fehlt**: `MixPreset.fadeInGain()` und `CrossfadeCurves.fadeInGain()` haben ausserhalb der Tests keinen Aufrufer. Seit der ADR-Konsolidierung (Schritt 18) gibt es keinen Dual-Player mehr, der eine Kurve anwenden koennte - Uebergaenge laufen als harter Wechsel. Der gewaehlte Stil wird gespeichert und wieder angezeigt, hoerbar ist er nicht. **B-AUD-5 (Weg b, Nutzerentscheidung 2026-09-11):** beide Bedienflaechen (Einstellungen -> Mix-Uebergaenge, Audio & DSP -> Crossfade) sind ausgegraut mit sichtbarem Hinweis "derzeit ohne Wirkung"; Stil und Dauer bleiben persistiert |
| 3 | UI: Abschnitt "Mix-Uebergaenge" in den Einstellungen — an/aus, Uebergangsstil (6 Chips mit Erklaertext), Dauer 1-12 s, Bit-Perfect-Hinweis; Strings DE/EN. Steuerung global statt pro Playlist (Nutzerentscheidung); BPM/Key-Badges folgen mit Phase 1 | UI vorhanden, aber seit B-AUD-5 (Weg b) ausgegraut mit Hinweis "derzeit ohne Wirkung" — sie steuert eine Wirkung, die es noch nicht gibt, und sagt das jetzt auch |
...
| 2 | Berechtigungs-UI + Badge im Train-Tab (2026-08-21, protokolliert in `docs/STATUS_FORTSCHRITT.md`): generischer Contract per Hilt-Qualifier injiziert, `rememberLauncherForActivityResult` ohne SDK-Leak, `HeartRateBadge` in der SensorCard (Puls-Chip / "Puls erlauben" / still bei fehlendem Provider), Refresh nach Dialog und Resume nur im Foreground; `FakeHeartRateSource` in `:core:testing`. Manifest nachgezogen (B-SEC-1, 2026-09-03): `READ_HEART_RATE` in `:data:health`, `HealthRationaleActivity` mit beiden Intent-Filtern in `:app` — vorher war der Knopf ohne Manifest-Deklaration wirkungslos | Abgeschlossen (Train-Tab, Geraeteabnahme des Dialogs ausstehend); Now-Playing-Einbindung folgt |
...
./gradlew :benchmarks:connectedBenchmarkAndroidTest   # Startup-Messung (verlangt ein zuvor erzeugtes Profil)
```
