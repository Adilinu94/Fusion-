...
| 2 | Berechtigungs-UI + Badge im Train-Tab (Verbesserungsplan Phase 5, 2026-08-21): generischer Contract per Hilt-Qualifier injiziert, `rememberLauncherForActivityResult` ohne SDK-Leak, `HeartRateBadge` in der SensorCard (Puls-Chip / "Puls erlauben" / still bei fehlendem Provider), Refresh nach Dialog und Resume nur im Foreground; `FakeHeartRateSource` in `:core:testing`. Manifest nachgezogen (B-SEC-1, 2026-09-03): `READ_HEART_RATE` in `:data:health`, `HealthRationaleActivity` mit beiden Intent-Filtern in `:app` — vorher war der Knopf ohne Manifest-Deklaration wirkungslos | Abgeschlossen (Train-Tab, Geraeteabnahme des Dialogs ausstehend); Now-Playing-Einbindung folgt |
