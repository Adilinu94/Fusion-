...
class PlaybackServiceBrowseTest {
    ...
    private fun callback(
        ...
        librarySessionRef = librarySession
        return cb
    }
    ...
    @Test
    fun `Root-Kinder sind die vier Kategorien`() =
        runTest {
            val cb = callback()
    ...
    @Test
    fun `Kategorie Titel listet verfuegbare Songs als playable Items`() =
        runTest {
            val cb = callback()
    ...
    @Test
    fun `Kategorie Alben listet Alben und Unterordner mit Songs`() =
        runTest {
    ...
    @Test
    fun `Kategorie Interpreten listet Interpreten und Unterordner mit Songs`() =
        runTest {
            val browse = FakeLibraryBrowseRepository()
    ...
    @Test
    fun `Kategorie Ordner listet Pfade und Unterordner mit Songs`() =
        runTest {
            val browse = FakeLibraryBrowseRepository()
    ...
    @Test
    fun `Unbekannte parentId liefert leere Liste`() =
        runTest {
            val cb = callback()
    ...
    @Test
    fun `onPlaybackResumption stellt Queue und Position wieder her`() =
        runTest {
            val state =
    ...
    @Test
    fun `onPlaybackResumption ohne gespeicherten Zustand wirft`() =
        runTest {
            val cb = callback()
            val error =
                runCatching {
...
private class FakeLibraryRepository(
    ...
    private val allSongs: List<Song> = librarySongs
    ...
    override suspend fun getSong(mediaStoreId: Long): com.dropsync.core.common.AppResult<Song> =
        allSongs
            ...
            ?: com.dropsync.core.common.AppResult.Failure(
                com.dropsync.core.common.AppError
                    .Unknown(debugMessage = null),
            )

    override val songs = flowOf(allSongs)

    override val availableSongs = flowOf(allSongs.filter { it.isAvailable })
    ...
    override suspend fun markUnavailable(mediaStoreId: Long) =
        com.dropsync.core.common.AppResult
            .Success(Unit)
...
private lateinit var librarySessionRef: MediaLibrarySession
